...
from data.init_admin import init_super_admin
from forms.user import RegisterForm, LoginForm
from forms.task import SubmissionForm
from forms.admin import TopicForm, TaskForm
...
# Назначение первого пользователя супер-админом
init_super_admin()
...
def tasks_by_topic(topic_id):
    with db_session.session_scope() as session:
        ...
        return render_template('tasks.html', title=topic.title, topic=topic,
                               tasks=tasks, task_status=task_status)
...
def task_detail(task_id):
    ...
    if form.validate_on_submit():
        ...
        # Запускаем проверку через Docker (вне сессии, т.к. может быть долгой)
...
def profile():
    with db_session.session_scope() as session:
        ...
        solved_task_ids = {sub.task_id for sub in successful_submissions}
...
def rating():
    with db_session.session_scope() as session:
        ...
        sorted_users = sorted(user_solved_counts.items(), key=lambda x: x[1][1], reverse=True)
        rating = [{'rank': idx, 'name': name, 'solved': cnt}
                  for idx, (_, (name, cnt)) in enumerate(sorted_users, 1)]
...
def toggle_admin(user_id):
    with db_session.session_scope() as session:
        ...
        if user and user.id != current_user.id:
            ...
            session.commit()
            flash(f'Права администратора для {user.name} {"назначены" if user.is_admin else "сняты"}', 'success')
        else:
...
def edit_topic(topic_id):
    with db_session.session_scope() as session:
        ...
        if form.validate_on_submit():
            ...
            session.commit()
            flash(f'Тема "{topic.title}" обновлена', 'success')
            return redirect(url_for('admin_topics'))
...
def delete_topic(topic_id):
    with db_session.session_scope() as session:
        topic = session.query(Topic).get(topic_id)
        if topic:
            # Удаляем задачи этой темы
            session.query(Task).filter(Task.topic_id == topic_id).delete()
            session.delete(topic)
            session.commit()
            flash(f'Тема "{topic.title}" и её задачи удалены', 'success')
    return redirect(url_for('admin_topics'))
...
def add_task():
    ...
    with db_session.session_scope() as session:
        topics = session.query(Topic).all()
        form.topic_id.choices = [(t.id, t.title) for t in topics]
    if form.validate_on_submit():
        try:
            json.loads(form.tests_json.data)
        except:
            flash('Неверный формат JSON в тестах', 'danger')
